...
from parcial import (
    parcial,  # Exibe o submenu de Resultados
    processar_submenu_resultados,  # Lida com Gerar Gráficos, Saldo por Jogador, Relatório
    processar_botao_parcial,       # Lida com datas
    processar_datas_intervalo
)

from matriz_jogador import (
    processar_menu_data_jogador,
    processar_botao_jogador,
    processar_datas_intervalo_jogador,
    processar_tipo_aposta_jogador
)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    keyboard = [
        ...
        # Removido o botão de Gráficos do menu principal:
...
async def processar_painel_principal(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "menu_produtos":
        ...
        # Agora o /parcial exibe o submenu de Resultados
...
def main():
    ...
    application.add_handler(CommandHandler("parcial", parcial))  # /parcial chama o submenu
    ...
    # Handler para submenu de Resultados (Parcial)
    application.add_handler(CallbackQueryHandler(processar_submenu_resultados, pattern="^sub_.*"))
    ...
    # Handler para o fluxo principal de "Matriz do Jogador" (escolha de data)
    application.add_handler(
        CallbackQueryHandler(
            processar_botao_jogador, 
            pattern=r"^jogador_(hoje|ontem|mes|7d|mes_anterior|intervalo)$"
        )
    )

    # Handler para digitação das datas (intervalo personalizado)
    application.add_handler(
        MessageHandler(
            filters.Regex(r"^datas_.*") & ~filters.COMMAND,
            processar_datas_intervalo_jogador,
            block=False
        )
    )

    # Handler para a escolha de tipo de aposta (Over, Under, All)
    application.add_handler(
        CallbackQueryHandler(
            processar_tipo_aposta_jogador,
            pattern=r"^jogador_tipo_.*"
        )
    )
    ...
    application.add_handler(
        CallbackQueryHandler(
            ...
            pattern=r"^liga_(battle|h2h|todas)\|(parcial|grafico|jogador)$"
        )
    )
